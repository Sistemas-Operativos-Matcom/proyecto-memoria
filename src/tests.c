...
void test_case_p_stress_mem() {
  ...
  ctx_switch(processes[0]);    
  ptr_t p0_x = mem_malloc(10);
  mem_push(100);
  ptr_t p1_x = mem_malloc(400);
...
void test_case_p_free_mem() {
  ...
  ptr_t p0_x = mem_malloc(10);
  mem_push(100);
  ptr_t p1_x = mem_malloc(200);
  ptr_t p2_x = mem_malloc(200);
...
void test_case_p_complex_procs() {
  ...
  ctx_switch(processes[1]);
  ptr_t p01_x = mem_malloc(10);
  mem_push(100);
  ptr_t p11_x = mem_malloc(200);
  mem_free(p1_x);
  ptr_t p21_x = mem_malloc(200);
  end_process(processes[0]);
  end_process(processes[1]);
  end_sim();
}
...
void test_case_p_freelist() {
  ...
  // Round 2
  ptr_t p05_x = mem_malloc(300);
  ptr_t p06_x = mem_malloc(10);
  m_free(p05_x);
  m_free(p06_x);

  ctx_switch(processes[1]);
  ptr_t p11_x = mem_malloc(50);
  ptr_t p12_x = mem_malloc(30);
  ptr_t p13_x = mem_malloc(60);
  ptr_t p14_x = mem_malloc(30);
  m_free(p12_x);
  m_free(p14_x);
  m_free(p13_x);
  m_free(p11_x);
  ...
  // Round 2
  ptr_t p15_x = mem_malloc(300);
  ptr_t p16_x = mem_malloc(10);
  m_free(p15_x);
  m_free(p16_x);
...
void test_case_no_memory() {
  setup_test_case(KB_SIZE(2), "case_no_memory");
  process_t processes[] = {
      PROCESS_FROM(0),
      PROCESS_FROM(1),
      PROCESS_FROM(2),
...
void test_case_stack_factorial(){
  setup_test_case(KB_SIZE(4), "case_stack_factorial");
  ...
  ctx_switch(processes[0]);           // Cambia de contexto al proceso 0
  mem_push(5);                 // Añade al stack el valor 70
  stack_factorial1();
  mem_pop_assert(factorial(5)); 
...
void test_case_proccess_stress() {
  setup_test_case(KB_SIZE(4), "case_proccess_stress");
  process_t processes[] = {
      PROCESS_FROM(0),
      PROCESS_FROM(1),
      PROCESS_FROM(2),
...
void run_tests(int argc, char **argv) {
  ...
  /*
  printf("**********  test_case_no_memory **********\n");
  test_case_no_memory();
  printf("**********  test_case_stack_factorial **********\n");
  test_case_stack_factorial();
  printf("**********  test_case_proccess_stress **********\n");
  test_case_proccess_stress();
  
  printf("**********  test_case_p_stress_mem **********\n");
  test_case_p_stress_mem();
  printf("**********  test_case_p_free_mem **********\n");
  test_case_p_free_mem();
  printf("**********  test_case_p_complex_procs **********\n");
  test_case_p_complex_procs();
  printf("**********  test_case_p_freelist **********\n");
  test_case_p_freelist(); */
