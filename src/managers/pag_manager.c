...
addr_t stack_pos[ProcAmountMax][MaxStackSize];
int stk_point[ProcAmountMax];
int curr_pid=0;
int lastp=-1;
...
void m_pag_init(int argc, char **argv) 
{
    ...
    memset(stk_point, 0, sizeof(stk_point[0]) * ProcAmountMax);

    for (int i = 0; i < ProcAmountMax; i++)
     {
        memset(stack_pos[i], 0, sizeof(stack_pos[i][0]) * MaxStackSize);
        memset(table[i].pf, 0, sizeof(table[i].pf[0]) * PagAmountMax);
        memset(table[i].spent, 0, sizeof(table[i].spent[0]) * PagAmountMax);
        memset(table[i].mk, 0, sizeof(table[i].mk[0][0]) * PagAmountMax * SizeOfPage);
    }
}
...
void m_pag_on_end_process(process_t process) {
    ...
    for(int i=0;i<PagAmountMax;i++) {
        if(table[pid].spent[i])
            for(int j=0;j<SizeOfPage;j++) from_pid[table[pid].pf[i]*SizeOfPage+j]=-1;
                  
    }
    memset(table[pid].pf, 0, sizeof(table[pid].pf[0]) * PagAmountMax);memset(stack_pos[pid], 0, sizeof(stack_pos[pid][0]) * MaxStackSize);
